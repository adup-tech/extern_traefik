...
import (
	"github.com/containous/traefik/vendor/github.com/stvp/go-udp-testing"
	"net/http"
	"testing"
	"time"
...
func TestStatsDWithPrefix(t *testing.T) {
	...
	statsdRegistry := RegisterStatsd(&types.Statsd{Address: ":18125", PushInterval: "1s", Prefix: "testPrefix"})
	defer StopStatsd()
	...
	expected := []string{
		// We are only validating counts, as it is nearly impossible to validate latency, since it varies every run
		"testPrefix.backend.request.total:2.000000|c\n",
		"testPrefix.backend.retries.total:2.000000|c\n",
		"testPrefix.backend.request.duration:10000.000000|ms",
		...
		"testPrefix.backend.server.up:1.000000|g\n",
	}
